...
def create_keyboard(response):
    ...
    if response == 'меню' or response == 'начать' or response == 'назад' or response == 'ютуб анал' or response == 'донат или реклама' or ('разраб' in response and response != 'разраб') or ('ютубер' in response and response != 'ютубер'):
    ...
    if event.type == VkEventType.MESSAGE_NEW:
        ...
        if event.from_user and not event.from_me:
            if response == "начать" or response == 'меню':
                ...
                text = 'Донат - https://www.donationalerts.com/r/eagle333333' + '\n' + 'Если хотите купить рекламу - напишите разрабу(меню -> связь с админами)'
                send_message(vk, 'user_id', id, message=text, keyboard=keyboard)
